...
Версия `0.1.0`. Доставка подтверждена на живых проектах Tracer 26–27.08.2026 на
всех трёх платформах: Android (устройство и эмулятор, включая нативный краш и
ANR), iOS (release-сборка на iPhone, без отладчика) и web (Chrome). Проверялись
не только доезд события, но и группировка, breadcrumbs, кастомные ключи и
остановка сбора.
[status.md](https://github.com/KonstantenKomkov/apptracer_flutter/blob/main/docs/status.md)
перечисляет каждый пункт и то, чем он подтверждён — там же осталось и
