...
Загружаются они одной командой из пакета, после сборки релиза:
...
Она сама найдёт `.dSYM` в архиве, возьмёт версию из `pubspec.yaml`, соберёт
архив и отправит его, а при отказе сервера завершится с ненулевым кодом — чтобы
релизный пайплайн останавливался, а не выпускал сборку, краши которой не
прочитать. Каталог, версию и номер сборки можно задать явно:
`--dir=…`, `--version=…`, `--build=…`; вместо `--token` понимается переменная
`TRACER_PLUGIN_TOKEN`. Место ей — там же, где вы собираете релиз: в CI или в
скрипте выпуска, чтобы не вспоминать о ней вручную.

Два варианта для тех, у кого уже есть своя оснастка. Если вы пользуетесь
**Fastlane** (инструмент автоматизации релизов iOS — если вы о нём не слышали,
значит не пользуетесь), у Tracer есть плагин для него. И у Tracer же есть
**Run Script фаза для Xcode**, которая срабатывает при архивации релиза. Оба
ставятся по документации вендора и делают ровно то же самое, что команда выше;
что-то одно, не всё сразу.
