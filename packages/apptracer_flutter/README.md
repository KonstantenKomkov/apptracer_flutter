...
это делает Firebase Crashlytics. Ничего вызывать не нужно, нужен только токен:

```sh
# ios/tracer_plugin_token — рядом с Podfile, добавьте в .gitignore
echo IOS_PLUGIN_TOKEN > ios/tracer_plugin_token
```

В CI файл не нужен: фаза читает `TRACER_IOS_PLUGIN_TOKEN` (или
`TRACER_PLUGIN_TOKEN`) из окружения сборки. Без токена она пишет предупреждение
и пропускает загрузку, а при неудачной отправке — предупреждение и продолжает:
ронять архив из-за сетевой ошибки хуже, чем собрать его без символов.
