...
**Руками это делать не надо.** У вендора есть Fastlane-плагин и Run Script фаза
для Xcode — она срабатывает на архивации релиза, и больше об этом можно не
думать; так же, как на Android всё делает Gradle-плагин. Ставится это по
документации Tracer, а не отсюда: пакет к загрузке символов отношения не имеет.

Если Fastlane не используется, в пакете есть команда, которая делает то же
самое:
...
Тот же запрос вручную, если своего пайплайна нет и ставить ничего не хочется:
